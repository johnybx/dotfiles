...
function M.setup(client, bufnr)
    if client.server_capabilities.codeLensProvider then
        vim.api.nvim_exec2(
            ...
            autocmd! * <buffer=%s>
            autocmd BufEnter ++once <buffer=%s> lua vim.lsp.codelens.enable(true, { bufnr = %s })
            autocmd CursorHold,InsertLeave <buffer=%s> lua vim.lsp.codelens.enable(true, { bufnr = %s })
            augroup END
            ]],
                bufnr,
                bufnr,
